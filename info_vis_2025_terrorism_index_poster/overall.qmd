...
```{r plot-with-black-line, echo=FALSE, fig.cap="Top 8 Countries: Terrorism Fatalities (2012–2022) with Guide in Caption"}
library(dplyr)
library(ggplot2)
library(scales)
library(tibble)
...
# 2) Order countries so the largest‐total sits at the bottom of the stack
order_levels <- df %>%
  group_by(country) %>%
...
# 3) Compute the annual global totals (for the black line)
top_k <- df %>%
  group_by(year) %>%
  summarise(global_k = sum(fatalities_k), .groups="drop")

# 4) Define key events (ypos slightly above the stack)
events <- tribble(
  ~year,  ~date,         ~label,                         ~ypos,
  2013,   "21 Aug 2013", "Ghouta chemical attack",       top_k$global_k[top_k$year==2013] + 1.5,
  2014,   "29 Jun 2014", "ISIS caliphate declared",      top_k$global_k[top_k$year==2014] + 3.5,
  ...
  2021,   "15 Aug 2021", "Taliban capture Kabul",         top_k$global_k[top_k$year==2021] + 1.5
)

# 5) Okabe–Ito color‐blind–safe palette
okabe_ito <- c(
  "Iraq"         = "#E69F00",
  "Afghanistan"  = "#56B4E9",
  "Nigeria"      = "#009E73",
