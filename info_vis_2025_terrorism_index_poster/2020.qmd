---
title: "Global Terrorism Index Analysis (2020)"
...
::: description-box
This analysis explores the Global Terrorism Index (GTI) data in 2020, examining trends and patterns in terrorism impact across countries. The GTI is a comprehensive measure that incorporates incidents, fatalities, injuries, and hostage situations to quantify the impact of terrorism globally.
:::
...
# Load the data
df <- read_excel("data/Global Terrorism Index 2023.xlsx", sheet = "2020")
...
# Filter top 15 countries based on 2020 score
top15 <- df_clean %>%
  arrange(desc(Score)) %>%
  slice(1:15)
...
::: description-box
The first visualization shows the overall trend of GTI scores for the top 15 most affected countries in 2020. This provides a comparative view of how terrorism impact has evolved over time across these nations.
:::

```{r}
#| label: overall-trend
#| fig-cap: "GTI Scores for Top 15 Most Affected Countries (2020)"

# Plot the data
ggplot(top15, aes(x = reorder(Country, Score), y = Score, fill = Country)) +
  geom_col() +
  coord_flip() +
  labs(title = "Top 15 Countries by Terrorism Index Score (2020)",
...
#| fig-cap: "Terrorism Index Trends by Country (2020), with Severity Levels"
#| fig-width: 12
#| fig-height: 8
...
ggplot(top15, aes(x = Country, y = Score)) +
  ...
  # Labels and titles
  labs(
    title = "Terrorism Index Scores by Country (2020)",
...
::: description-box
This visualization shows the countries with the highest number of terrorism-related fatalities in 2020. The number of fatalities is a direct measure of the human cost of terrorism.
:::

```{r}
#| label: fatalities
#| fig-cap: "Countries with Highest Terrorism-Related Fatalities (2020)"
#| warning: false
...
ggplot(top_deaths, aes(x = reorder(Country, Fatalities), y = Fatalities)) +
  ...
  labs(title = "Top 15 Countries by Total Fatalities (2020)",
...
```{r}
#| label: injuries
#| fig-cap: "Countries with Highest Terrorism-Related Injuries (2020)"
#| warning: false
...
ggplot(top_injuries, aes(x = reorder(Country, Injuries), y = Injuries)) +
  ...
  coord_flip(clip = "off") +  # Prevent clipping of labels
  scale_y_continuous(limits = function(x) c(0, x[2] * 1.1)) +  # Expand y-axis to make room for labels
  labs(title = "Top 15 Countries by Total Injuries (2020)",
...
```{r}
#| label: incidents
#| fig-cap: "Countries with Highest Number of Terrorist Incidents (2020)"
#| warning: false
...
ggplot(top_incidents, aes(x = reorder(Country, Incidents), y = Incidents)) +
  ...
  coord_flip(clip = "off") +  # Prevent clipping of labels
  scale_y_continuous(limits = function(x) c(0, x[2] * 1.1)) +  # Expand y-axis to make room for labels
  labs(title = "Top 15 Countries by Total Incidents (2020)",
...
```{r}
#| label: hostages
#| fig-cap: "Countries with Highest Number of Hostages in Terrorist Incidents (2020)"
#| warning: false
...
ggplot(top_hostages, aes(x = reorder(Country, Hostages), y = Hostages)) +
  ...
  coord_flip(clip = "off") +  # Prevent clipping of labels
  scale_y_continuous(limits = function(x) c(0, x[2] * 1.1)) +  # Expand y-axis to make room for labels
  labs(title = "Top 15 Countries by Total Hostages (2020)",
...
```{r}
#| label: bubble-chart
#| fig-cap: "Multi-Dimensional Analysis of Terrorism Impact by Country (2020)"
#| fig-width: 11
#| fig-height: 8
...
ggplot(bubble_data, aes(x = Fatalities, y = Injuries, size = Score, label = Country)) +
  ...
  labs(title = "Bubble Chart: Fatalities vs Injuries by Country (2020)",
